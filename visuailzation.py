...
checkpoint = './checkpoint_ssd300_200.pth.tar'
...
def run(original_image, min_score, max_overlap, top_k, suppress=None):
    ...
    for i in range(det_boxes.size(0)):
        ...
        if det_labels[i] != "background":
            ...
            box.append(det_boxes[i])


    del draw

    return annotated_image, box, det_labels, det_scores
...
if __name__ == '__main__':
    ...
    with open(os.path.join(root + "a_annotations.json"), "r") as f:
        ...
        for v, k in enumerate(dict):
            ...
            #     det_boxes[n][2], det_boxes[n][3] = \
            #         det_boxes[n][2]-det_boxes[n][0], \
            #         det_boxes[n][3]-det_boxes[n][1]
            #     annotations.append({"image_id": k["id"],
            #                         "bbox": det_boxes[n],
            #                         "category_id": det_labels[n],
            #                         "score": det_scores[0][n].item()})
            # number += 1
            # print(number)
